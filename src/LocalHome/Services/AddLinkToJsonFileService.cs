...
namespace LocalHome.Services
{
    public class AddLinkToJsonFileService : IAddLinkToJsonFileService
    {
        ...
        public async Task AddLinkAsync(string jsonFilePath, UserLink userLink, string groupname)
        {
            ...
            existingLinks.FirstOrDefault(item => item.Name.Equals(groupname)).Links.Add(userLink);
